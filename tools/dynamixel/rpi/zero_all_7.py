#!/usr/bin/env python3
"""Move seven configured XL330 motors, one at a time, to encoder position 0.

For XL330 Extended Position / Current-based Position modes, this means the
nearest whole-turn position whose value modulo 4096 is 0. For normal Position
Control Mode, the target is literal 0.
"""
...
def base_target(present: int, mode: int) -> int:
    """Return the closest coordinate that represents encoder position zero."""
    if mode == 3:
...
def main() -> int:
    ...
    try:
        ...
        controller.close()
    print("All seven motors are at encoder zero (position mod 4096 = 0).")
    return 0
