...
저는 **Blender 모델링·3D 프린팅, U2D2 통신·7개 모터 동기 구동·키보드 제어, ROS 브릿지와 Cyclone DDS 통신 설정, 고정부·스풀·텐던 통합**을 담당했습니다. 손동작 인식, 명령 중재·guard·안전 상태, 관제 UI·기록 등은 팀원이 구현했습니다. 아래 표에는 제가 작성한 모터 점검 코드의 세부 처리를 정리했습니다.

| 확인한 문제 | 구현한 처리 | 확인 범위 |
| --- | --- | --- |
| 토크를 켤 때 이전 목표로 움직일 수 있음 | 이동 함수에서 현재 위치를 Goal Position에 먼저 기록 → Torque ON → 새 목표 적용 | 모터 점검 코드의 실행 순서 |
| 다회전 위치에서 원위치 복귀 경로가 길어짐 | 현재 위치에 가장 가까운 `2048 + 4096 × k` 선택 | 중앙각 복귀이며 손가락 끝점 보정과 구분 |
| 같은 엔코더 명령도 기구 상태에 따라 굽힘량이 달라짐 | 모터별 점검과 아크릴 고정부·스풀·텐던 조립, 권취·배선 경로 기록 | 최대 가동 범위 간섭·재조립 반복성은 추가 검증 필요 |

관절각·필터·모터 보정식, 토크 인가 순서와 실제 검증 범위는
[구동·기구 통합 기술 노트](docs/engineering-notes.md)에 정리했습니다.
...
- [구동·기구 통합 기술 노트](docs/engineering-notes.md)
