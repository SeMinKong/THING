  ...
  if (safetyState.state === "HOLD") {
    ...
    messages.push({
      ...
      text: `마지막 명령으로부터 ${t.SAFE_DEADLINE_MS}ms(설정값) 안에 재개되지 않으면 `
        ...
        + "지금 정지(STOP)를 요청하면 제어기 확인 뒤 정지 절차(RESET)를 거쳐 준비 상태로 돌아갑니다.",
      actionNeeded: "STOP",
    });
  }

  // V7 신규. 명시적 STOP 뒤 settle·torque-off 확인 구간이며 정상 절차다.
  if (safetyState.state === "RESET") {
    messages.push({
      level: "warning",
      text: `정지 절차를 진행 중입니다. 설정된 안정 시간(${SPEC.STOP_SETTLE_MS}ms)과 `
